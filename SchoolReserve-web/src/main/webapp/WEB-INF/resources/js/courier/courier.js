...
var courierGrid = new Ext.grid.GridPanel({
			id : 'courierGrid',
			plugins : [rowEditing],
			store : store,
			region : 'center',
...
courierGrid.getSelectionModel().on('selectionchange',
		function(selModel, selections) {
	courierGrid.down('#delete').setDisabled(selections.length === 0);
		});
...
var clearForm = function() {
	...
	courierForm.getForm().reset();
}

var queryForm = function() {
	Ext.Msg.alert('查询', '将开始执行查询！');
}
var courierForm = new Ext.form.FormPanel({
			title : '信息查询',
			width : 200,
			height : 200,
			frame : true,
